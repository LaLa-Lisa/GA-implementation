...
void LGenetic::learn(int generationsNum) {
	...
	std::ofstream graph_out("genetic16_graph.txt");
	max_iteration = generationsNum;
	for (int t = 0; t < generationsNum; ++t)
	{
		current_iteration = t;
		sort();
		std::cout << t << " - " << fitness_function(b_gene) << " loss - " << loss(b_gene, file_validation) << "\n";
		graph_out << t << " " << fitness_function(pop[0]) << "\n";
		if (minn > fitness_function(pop[0]))
		{
			minn = fitness_function(pop[0]);
			//std::cout << t << "  " << minn << "\n ";
			//mn.push_back(minn);
			//index.push_back(t);
